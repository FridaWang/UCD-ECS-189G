# Task1 accurancy: 0.815500
# Task2 accurancy: 0.808000
# Task3 accurancy: 0.837500(set Alpha = 4)
# Task4 accurancy: 0.850500(set Alpha = 4, using NOT_ strategy for the following 4 words after detecting a negation, also using weighted words strategy)
...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.strongWeight = 0.6
        self.weakWeight = 0.3
        #self.strongWeightedWords = set()
        #self.weakWeightedWords = set()
    ...
    def classify(self, words):
        ...
        if self.bestModel ==True:
            words = self.negationProcess(words)
    ...
    def negationProcess(self,words):
        negationWords = ["never","neither","nor","nobody","nowhere","not","no","none"]
        negation = False
        count = 0
        for i in range(len(words)):
            if negation and count < 4:
                words[i] = "NOT_" + words[i]
                count += 1
            if words[i] in negationWords or words[i][-3:] == "n\'t":
                negation = True
            if count == 4:
                negation = False
                count = 0
