...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1): #ignore <s> and </s>
      ...
      word = sentence[i]
      # Get all edits and associated log probabilities
      edits = self.editModel.editProbabilities(word)
      # For each possible new edit
      for edit in edits:
        # Form a new sentence by replacing the original word with new edit.
        trialSentence = sentence[:]
        trialSentence[i] = edit[0]
        # The score for this edit = log(scentenceScore) + log(editScore)
