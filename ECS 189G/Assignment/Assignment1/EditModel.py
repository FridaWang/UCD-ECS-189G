...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1, len(word) + 1):
      #The corrupted signal is just the preceding character
      corruptLetters = word[i-1] 
      #The correct signal are the preceding character and the character to be inserted
      for ch in EditModel.ALPHABET:
        correctLetters = corruptLetters + ch
        #The corrected word inserts character i (and lacks the start symbol)
        correction = "%s%s%s" % (word[1:i], ch, word[i:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      ...
      for ch in EditModel.ALPHABET:
        correctLetters = ch
        if correctLetters == corruptLetters:
          continue
        #The corrected word inserts character i (and lacks the start symbol)
